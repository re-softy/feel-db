...
const emotions = [
  ...
  { label: "Surprise", svg: "/emotions/surprise.svg" },
...
function RateMovie({
  ...
  const gridClass = rows === 1 ? "grid-cols-1 md:grid-cols-2 md:gap-x-6 lg:grid-cols-1 overflow-y-auto pr-2 max-h-64 sm:max-h-80 md:max-h-96 lg:max-h-[400px] xl:max-h-[500px]" : "grid-cols-1 xl:grid-cols-2 gap-x-6 gap-y-[1px] h-[330px]";
  ...
  return (
    <div className={`rounded-lg ${className} ${border ? 'hidden lg:block border border-[#262626] p-6' : ''}`}>
      ...
      {!border && <p className="text-xl mb-2 font-medium">Rate the Movie</p>}
